...
%%% SVG images: inkscape is not available; instead, build/media/*.svg are pre-rasterised
%%% to PNG by rsvg-convert in the Makefile. Redefine \includesvg to call \includegraphics
%%% with the PNG version (same path, .png extension) via \filename@parse.
%%% \makeatletter must wrap the \renewcommand definition so \filename@parse is tokenised
%%% as a single control sequence while @ is a letter.
\makeatletter
\AtBeginDocument{%
