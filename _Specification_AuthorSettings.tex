...
%%% SVG images: pdflatex is not run with -shell-escape, so the svg package
%%% cannot invoke inkscape itself. build-pdf.sh pre-rasterises
%%% build/media/*.svg to PNG via rsvg-convert; redefine \includesvg to call
%%% \includegraphics with the PNG version (same path, .png extension) via
%%% \filename@parse. \makeatletter must wrap the \renewcommand definition so
%%% \filename@parse is tokenised as a single control sequence while @ is a letter.
\makeatletter
\AtBeginDocument{%
